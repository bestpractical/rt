...
`jquery/`). A handful retain the major version in the name
(`chart-js-2/`, `ckeditor4/`, `dropzone-4/`) — kept when the upstream
...
`COPYING.GPL` for CKEditor 4, where `LICENSE.md` is the upstream-shipped
...
- `true` examples: `ckeditor4` (parses HTML), `jquery` (selector
  parsing), `dropzone-4` (handles user files), `bootstrap-select`
  (autocomplete on typed input).
- `false` examples: `chart-js-2` (renders app-supplied data), `d3`
  (presentational), `mousetrap` (keyboard binding), `fontawesome`
  (static icons).
...
inactive original instead of the original itself, point
...
arrangement; mention it in `notes:`. See `ckeditor4/` for a worked
example.
...
variable in `etc/RT_Config.pm.in`), the directory still holds
`metadata.yml` plus license info. `artifacts:` may be `[]`; a stub
`README` should explain where the content actually lives.
