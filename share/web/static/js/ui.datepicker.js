 ...
 * sunnavy@bestpractical modified this so we can pick time too!
 */
...
$.extend(Datepicker.prototype, {
	...
	_hideDatepicker: function(input, speed) {
		...
		if (rangeSelect && this._stayOpen)
			this._selectDate(inst, inst._formatDate(
                inst._currentSecond, inst._currentMinute, inst_currentHour,
	...
	_selectTime: function(id, select, period) {
        ...
        var value = 
			select.options[select.selectedIndex].value - 0;
        if ( period == 'S' ) {
		    inst._selectedSecond = inst._currentSecond = value;
        }
        else if ( period == 'I' ) {
		    inst._selectedMinute = inst._currentMinute = value;
        }
        else {
		    inst._selectedHour = inst._currentHour = value;
        }
        this._stayOpen = true;
		this._doNotHide = true;

		this._selectDate(id, inst._formatDate(
            inst._currentSecond, inst._currentMinute, inst._currentHour,
			inst._currentDay, inst._currentMonth, inst._currentYear));
        this._stayOpen = false;
	},
	...
	_selectDay: function(id, month, year, td) {
		...
		this._selectDate(id, inst._formatDate(
			...
			inst._currentDay, inst._currentMonth, inst._currentYear));
		if (this._stayOpen) {
			inst._rangeStart = new Date(inst._currentYear, inst._currentMonth, inst._currentDay, inst._currentHour, inst._currentMinute, inst._currentSecond);
			this._updateDatepicker(inst);
		}
		else if (rangeSelect) {
	...
	_possibleChars: function (format) {
		...
		for (var iFormat = 0; iFormat < format.length; iFormat++)
			if (literal)
				...
				switch (format.charAt(iFormat)) {
                    case 'd': case 'm': case 'y': case 'h': case 'i': case 's':
						chars += '0123456789'; 
						break;
					case 'D': case 'M':
...
$.extend(DatepickerInstance.prototype, {
	...
	_setDateFromField: function(input) {
		...
		this._endSecond = this._endMinute = this._endHour = this._endDay = this._endMonth = this._endYear = null;
		var date = defaultDate = this._getDefaultDate();
		if (dates.length > 0) {
			var settings = this._getFormatConfig();
			if (dates.length > 1) {
				date = $.datepicker.parseDate(dateFormat, dates[1], settings) || defaultDate;
                this._endSecond = date.getSeconds();
                this._endMinute = date.getMinutes();
                this._endHour = date.getHours();
		...
		this._currentHour = (dates[0] ? date.getHours() : 0);
		this._currentMinute = (dates[0] ? date.getMinutes() : 0);
		this._currentSecond = (dates[0] ? date.getSeconds() : 0);
		this._adjustDate();
	},
	...
	_generateDatepicker: function() {
		var today = new Date();
		today = new Date(today.getFullYear(), today.getMonth(), today.getDate(), today.getHours(), today.getMinutes(), today.getSeconds()); // clear time
		...
		for (var row = 0; row < numMonths[0]; row++)
			for (var col = 0; col < numMonths[1]; col++) {
				...
				html += '<div class="ui-datepicker-one-month' + (col == 0 ? ' ui-datepicker-new-row' : '') + '">' +
					this._generateMonthYearHeader(drawSecond, drawMinute, drawHour, drawMonth, drawYear, minDate, maxDate,
	...
	_generateMonthYearHeader: function(drawSecond, drawMinute, drawHour, drawMonth, drawYear, minDate, maxDate, selectedDate, secondary) {
		...
		{
			html += '<br />';
			html += '<select class="ui-datepicker-new-hour" ' +
				'onchange="jQuery.datepicker._selectTime(' + this._id + ', this, \'H\');" ' +
				'onclick="jQuery.datepicker._clickMonthYear(' + this._id + ');"' +
			...
			html += '</select>';
			html += '<select class="ui-datepicker-new-minute" ' +
				'onchange="jQuery.datepicker._selectTime(' + this._id + ', this, \'I\');" ' +
				'onclick="jQuery.datepicker._clickMonthYear(' + this._id + ');"' +
			...
			html += '</select>';
			html += '<select class="ui-datepicker-new-second" ' +
				'onchange="jQuery.datepicker._selectTime(' + this._id + ', this, \'S\');" ' +
				'onclick="jQuery.datepicker._clickMonthYear(' + this._id + ');"' +
				(showStatus ? this._addStatus(this._get('secondStatus') || '&#xa0;') : '') + '>';
	...
	_formatDate: function(second, minute, hour, day, month, year) {
		if (!day) {
