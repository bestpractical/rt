        ...
        if ( ( $HTML::Mason::Commands::DECODED_ARGS->{'id'} || '' ) =~ /^(\d+)$/ ) {
            ...
            if ( $obj and $obj->id ) {
                ...
                if ( defined $HTML::Mason::Commands::session{"collection-RT::Tickets"} ) {
                    ...
                    if ( $item_map->{$id}->{prev} ) {
                        $search->child(
                            first        => title => q{<span class="fas fa-angle-left"></span>},
                            escape_title => 0,
                            class        => "nav",
                        ...
                        $search->child(
                            prev         => title => q{<span class="fas fa-angle-double-left"></span>},
                            escape_title => 0,
                            class        => "nav",
            ...
            || $class eq 'RT::Assets' && $request_path =~ m{^/Asset/(?!Search/)} )
        {
            ...
            if ( $search_results_page_menu && $has_query ) {
                $search_results_page_menu->child(
                    ...
                    attributes     => {
                        'data-toggle'         => 'tooltip',
                        'data-original-title' => loc('Search Results'),
                        alt                   => loc('Search Results'),
                    },
                );
            }
        }
