...

{
    ...
    sub new {
        ...
        my %args  = (
            app =>
                RT::REST2->PSGIWrap( sub { die "Requested non-REST path" } ),
            @_,
        );
    ...
    sub hypermedia_ref {
        ...
        Test::More::is( @matches, 1,
            "got one match for hypermedia with ref '$ref'" )
            or return;
        return $matches[0];
    ...
    # Generalized method to handle HTTP requests and logging
...
# Handle different cases where request parameters differ (like POST with Content)
        ...
        for ( my $i = 0; $i < @params; $i++ ) {
            ...
            if ( ref $key eq 'HASH' ) {
                ...
                $i++;    # Move the index forward to skip the next element
            } else {

                # Assume this is a scalar key, so pair it with the next value
                $params_hash{$key} = $params[ $i + 1 ];
                $i++
                    ; # Move the index forward to skip the value we just paired
            }
        }
        ...
        delete $params_hash{Content};    # Separate headers if any
        ...
        push @requests_log, {
            ...
            response => {
                ...
                headers => $response
                    ->headers_as_string,    # Response headers as a string
            }
        };
