...
try to stifle your creativity, but to make life easier for everyone who
has to work with your code, and to aid those who are not quite sure how
to do something.

The conventions below apply to Perl modules, web programs, and
...
a need to break one of the rules herein, however, then it is
...
A quick and easy way to try out changes in RT is to use SQLite and
the inplace layout so you can run code inside your git checkout. Here
is a sample configuration you can use. Put this in a file under
C<etc/RT_SiteConfig.d>, say C<etc/RT_SiteConfig.d/dev.pm>:
...
If the Mason cache is enabled (i.e, you haven't set C<$DevelMode>), then as you
make changes you will likely need to clear the cache:

    rm -rf var/mason_data

You may also need to clear the database as you adjust, test things etc. By
default, the SQLite database file is created in C<var/rt5>. Just delete that
file to clear the test database, and re-run C<make initdb> to start fresh.

You can also run development checkouts with all the databases RT supports.
Change the C<configure.ac> options above to set the database you want to
use. That will change the dependencies installed by C<make fixdeps>, so
you need to re-run that and install the drivers for the new database.
...
All command-line program options should be documented with a
command-line help option showing usage and all options.
...
Unlike messages meant for the user, log messages are not to be
internationalized.
...
Used to report issues that need attention, but are not failures. Many systems
will run production at the "warn" level, so only use this for important
events.
...
outside the current object.  This is not to be enforced by the code
...
The RT codebase is more than 25 years old; as such, there are sections
...
RT includes a F<.perltidyrc> in its top-level which encodes many of
the conventions. When in doubt, use perltidy.

=head2 Indentation and Blank Space
...
=head2 Translating Hard-coded Strings
...
=head2 Translation Comment Markers

C<#loc_pair> is used for declaring that both the key and value
...
=head2 String Extraction in Web Templates
...
RT comes with a fairly complete test suite. The C<*-trunk> and
...
=head2 Playwright Tests

Starting with version 6.0.3, RT uses Playwright for browser-based testing.
Playwright allows you to test functionality with a virtual browser,
making it possible to test behavior that happens only in the browser,
specifically javascript features. This is also helpful for confirming
htmx interactions. Selenium was used in previous versions.

Playwright tests are located in the C<t/playwright/> directory.
...
=head3 Running Playwright Tests
...
You can replace $GITHUB_WORKSPACE with your current working directory if you are
...
RT's source code is stored in a C<git> repository.  The RT source repository
is available via git from GitHub; you can browse it at
L<http://github.com/bestpractical/rt/> or obtain a local copy via:

    git clone git://github.com/bestpractical/rt.git

=head2 Branch Organization
...
diffs, you can use a configuration like this after installing the
