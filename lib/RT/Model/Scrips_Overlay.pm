...
sub new_item  {
  ...
  return(new RT::Model::Scrip($self->CurrentUser));
}
# }}}
...
sub _setupSourceObjects {
    ...
    if ( ( $self->{'TicketObj'} = $args{'TicketObj'} ) ) {
        ...
        $self->{'TicketObj'} = RT::Model::Ticket->new( $self->CurrentUser );
    ...
    if ( ( $self->{'TransactionObj'} = $args{'TransactionObj'} ) ) {
        ...
        $self->{'TransactionObj'} = RT::Model::Transaction->new( $self->CurrentUser );
