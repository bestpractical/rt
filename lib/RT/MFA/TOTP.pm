...
=head2 QRCodeSVG Secret => $secret, User => $user_obj

Returns the QR code as inline SVG markup (a C<< <svg>...</svg> >> string)
suitable for embedding directly in an HTML page. The C<otpauth://> URI is
encoded into the QR modules, not into any attribute, so the markup contains
...
sub QRCodeSVG {
    ...
    my $rows = Text::QRCode->new( level => 'M', casesensitive => 1 )->plot($uri);

    my $modules = scalar @$rows;    # QR codes are square
    my $margin  = 4;                # quiet zone width, in modules (per QR spec)
    my $dim     = $modules + 2 * $margin;

    my $rects = '';
    for my $y ( 0 .. $#$rows ) {
        my $row = $rows->[$y];
        for my $x ( 0 .. $#$row ) {
            next unless $row->[$x] eq '*';
    ...
    return
          qq{<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" }
        . qq{viewBox="0 0 $dim $dim" shape-rendering="crispEdges">}
        . qq{<rect width="$dim" height="$dim" fill="#ffffff"/>}
        . qq{<g fill="#000000">$rects</g>}
        . q{</svg>};
}
