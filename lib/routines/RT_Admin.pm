...
sub add_modify_queue_conf {
    ...
    $queue_id=&scrub($in_queue_id); # if we did in_queue_id, the test below would fail.
    $in_mail_alias=&scrub($in_mail_alias);
	...
	if ($users{$in_current_user}{admin_rt}) {
	    ...
	    system("cp", "-rp", "$template_dir/originals","$template_dir/queues/$in_queue_id");
	    &rt::load_queue_conf();
...
sub add_modify_user_info {
    ...
    $new_user_id = &scrub($in_user_id);
    $new_password =&scrub($in_password);
    $new_email = &scrub($in_email);
    $new_phone = &scrub($in_phone);
    $new_office = &scrub ($in_office);
    $new_comments = &scrub ($in_comments);
