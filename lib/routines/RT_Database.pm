#
# RT is copyright 1997 Jesse R Vincent (jrvincent@wesleyan.edu) and is 
...
sub add_request {
    ...
    $in_queue_id = &scrub($in_queue_id);
    $in_area = &scrub($in_area);
    $in_requestors = &scrub($in_requestors);
    $in_alias = &scrub($in_alias);
    $in_owner = &scrub($in_owner);
    $in_subject = &scrub($in_subject);
    $in_status = &scrub($in_status);
    $in_current_user = &scrub($in_current_user);
...
sub add_transaction {
    ...
    $in_actor = &scrub($in_actor);
    $in_type = &scrub($in_type);
    $in_data = &scrub($in_data);
