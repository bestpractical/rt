...
ok(
    $pg1->principal_object->has_right(
        Right  => 'ShowConfigTab',
        Object => RT->system
    ),
...
( $ret, $msg ) = $ace->Delegate( principal_id => $pg1->principal_id );
ok( $ret, "Delegate ShowConfigTab to pg1: $msg" );
( $ret, $msg ) = $g1->principal_object->RevokeRight( Right => 'DelegateRights' );
...
( $ret, $msg ) = $g2->principal_object->GrantRight( Right => 'AdminGroup',
					        Object => $pg1);
...
( $ret, $msg ) = $g1->delete_member( $u1->principal_id );
ok( $ret, "Delete test user 1 from g1: $msg" );
ok( $pg1->principal_object->has_right(Right  => 'AdminGroup',
				Object => $pg1),
    "Test personal group 1 retains AdminGroup right on pg1 after user removed from g1" );
( $ret, $msg ) = $u1->principal_object->RevokeRight( Right => 'DelegateRights',
						Object => RT->system );
ok( not ($pg1->principal_object->has_right(Right  => 'AdminGroup',
				     Object => $pg1)),
...
( $ret, $msg ) = $g2->principal_object->GrantRight( Right => 'ShowConfigTab' );
ok( $ret, "Grant ShowConfigTab to g2: $msg" );
( $ret, $msg ) = $g1->AddMember( $g3->principal_id );
ok( $ret, "Add g3 to g1: $msg" );
( $ret, $msg ) = $g3->AddMember( $u1->principal_id );
...
( $ret, $msg ) = $pg1->AddMember( $u2->principal_id );
ok( $ret, "Add test user 2 to pg1: $msg" );
( $ret, $msg ) = $g2->AddMember( $u2->principal_id );
...
ok( $pg2->principal_object->has_right(Right  => 'ShowConfigTab',
				 Object => RT->system),
