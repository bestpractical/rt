...
diag "make sure skipped order by field doesn't break search";
{
    ...
    $agent->follow_link_ok({id => 'page-results'});
    $agent->content_contains(q{class="bi bi-sort-up"}, 'Default order is ASC');
    $agent->follow_link_ok({text_regex => qr/^Requestor/});
    $agent->content_contains(q{class="bi bi-sort-down"}, 'Order changed to DESC');
    $agent->follow_link_ok({text_regex => qr/^Requestor/});
    # No sort icons, just Requestor
    $agent->follow_link_ok({text => 'Requestor'});
    $agent->content_contains(q{class="bi bi-sort-up"}, 'Order changed to ASC');
}
