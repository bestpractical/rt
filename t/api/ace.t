...
$user_a_equiv_group->load_acl_equivalence_group($user_a->PrincipalObj);
ok ($user_a_equiv_group->id, "Loaded the user A acl equivalence group");
my $user_b_equiv_group = RT::Model::Group->new($user_b);
$user_b_equiv_group->load_acl_equivalence_group($user_b->PrincipalObj);
ok ($user_b_equiv_group->id, "Loaded the user B acl equivalence group");
$own_ticket_ace->load_by_values( PrincipalType => 'Group', PrincipalId => $user_a_equiv_group->PrincipalId, Object=>$q, RightName => 'OwnTicket');
...
ok($user_a->has_right( Object => RT->System, Right => 'DelegateRights')    ,"user a has the right 'AdminOwnPersonalGroups' directly");
...
$delegated_ace->load_by_values ( Object => $q, RightName => 'OwnTicket', PrincipalType => 'Group',
...
ok(    $a_delegates->delete_member($user_b->PrincipalId)  ,"user a removes b from pg 'delegates'");
...
$group_ace->load_by_values( PrincipalType => 'Group', PrincipalId => $del1->PrincipalId, Object => $q, RightName => 'OwnTicket');
...
($val, $msg) = $del2b->delete_member($user_a->PrincipalId);
...
($val, $msg) = $del2->delete_member($user_a->PrincipalId);
...
$del2_right->load_by_values( PrincipalId => $del2->PrincipalId, PrincipalType => 'Group', Object => $q, RightName => 'OwnTicket');
...
ok( $user_a->has_right(Right => 'OwnTicket', Object => $q)   ,"user b has the right to own tickets in queue 'DelegationTest'");
