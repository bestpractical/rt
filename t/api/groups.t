...
my $testuser = RT::Model::User->new(RT->system_user);
($id,$msg) = $testuser->create(Name => 'JustAnAdminCc');
ok ($id,$msg);

my $global_admin_cc = RT::Model::Group->new(RT->system_user);
...
$groups = RT::Model::GroupCollection->new(RT->system_user);
$groups->WithRight(Right => 'OwnTicket', Object => $q);
ok ($id,$msg);
is($groups->count, 3);

my $RTxGroup = RT::Model::Group->new(RT->system_user);
