...
There are different rights you can grant to allow users access to the
...
After creating a new dashboard, you can access the "Content" submenu, where you
will be able to add widgets to your dashboard. However, before we do that, you
need to set up the layout of your dashboard.

You can start by adding a new Row to the Page Layout by clicking the plus sign
in the top right corner of the page. This will bring up a popup where you can
set a Title for that section of the dashboard and specify the number of columns
you want to use.
...
layout is built with Bootstrap's grid system, and the width of the columns should
sum to 12. If you want to create a 3-column layout, the system will automatically
set the Layout to "col-4", which means that each of the 3 columns will have a
width of 4 units in the grid system. You can also set the width of each column to
be different. For example, if you want a layout with 2 smaller sidebars and a
larger main column in the middle, you can set the layout to "col-3,col-6,col-3".

=head3 Creating a Personal Dashboard

Now, to show some additional dashboard features, we'll set up a dashboard and
notifications to track outstanding invoice tickets.
...
and save a search that displays the
...
Create a new Dashboard similarly to how we did in the
L</"Creating a Dashboard with existing Widgets"> section. Type "Outstanding 
...
Add a new Row to the Page Layout as described in L</"Page Layout">.
You can set the title to "Outstanding Invoices" and set the number of columns
to 1 or 2. Adjust the layout if you'd like and click "Add".
...
menu, select Reports > Actions ->
"Update This Menu" or "Logged in as" > Settings > "Modify Reports menu". You'll
see a page that allows you to drag and drop the dashboards and reports you want
to appear under your Reports menu.
...
Superusers automatically have access to all dashboards in the system.
