...
code in your templates. If you click the "Source" button in the editor,
you might see your C<$Ticket-E<gt>Id> appear like this:

    { if ( $Ticket &amp;&amp; $Ticket-&gt;Id ) {

RT will try to convert these automatically and make the code run.

Quotation marks can be a challenge because the editor can turn them into
Unicode. To avoid this issue, you can use a Perl feature that allows you
to use alternate syntax for quotation marks. One option is the C<qq>
operator with any delimiter. So if you want to type this:
...
And you are getting errors, try converting the quotation marks like this:
...
The above uses a "pipe" or vertical line after C<qq> as the start and end
because it's unlikely to clash with HTML or other Perl code.
