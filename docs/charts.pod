...
the users staff1 and staff2 both assigned, or even one or more groups. In
...
You can also group by custom roles. Note that these group-by charts do a
"shallow" search, meaning they group by direct members of the role and do
not go further down to chart members of groups who are assigned to a role.

In addition to grouping by standard user fields like Name and EmailAddress,
you can group by custom fields applied to users. For example, if you have
a user custom field called "Department", you can group tickets by the
department of their Requestors by selecting "Requestor" from the first
Group By dropdown and "CustomField.{Department}" from the second dropdown.
This allows you to create charts showing ticket distribution across
organizational units based on who submitted the tickets.
