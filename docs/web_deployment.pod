...
To run RT as a Plack standalone server (e.g., L<Starman>) on a local
port behind an HTTPS-terminating reverse proxy, enable
C<$WebReverseProxy> in F<RT_SiteConfig.pm>:

    Set($WebReverseProxy, 1);

RT then honors the C<X-Forwarded-*> request headers to determine the
public-facing scheme, host, port, and client IP. Your proxy must
B<set or overwrite> these headers, not just pass client values
through. FastCGI and mod_perl deployments do B<not> need this
option, because the frontend already passes C<SERVER_PORT>,
C<SERVER_NAME>, and C<HTTPS> directly via the FastCGI or mod_perl
protocol.
...
C<mod_proxy_http>'s C<ProxyPass> automatically adds
C<X-Forwarded-For>, C<X-Forwarded-Host>, and C<X-Forwarded-Server>,
but not C<X-Forwarded-Proto>. Enable C<mod_headers> (e.g.,
C<a2enmod headers> on Debian/Ubuntu) and add the missing header to
the TLS vhost:
