...
Tickets are shaded based on the date selected. Starts is green, Due is amber,
etc.

If you create a display format that contains Starts and Due, or Started and
Resolved, the calendar will use those two dates as a beginning and end,
and display the ticket as a multiple day event.
