...
the inplace layout so we use the code inside the git checkout. Here
...
You may also need to delete the database as you adjust, test things etc. If you
delete this file, then make sure to re-run C<rt-setup-database> as above! You
can delete the database with:
