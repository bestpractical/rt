...
Each widget that supports C<Default> feature as well has C<default_label> and
C<default_value> arguments.
...
As well when you show a widget you should pass undef as C<current_value>
...
=head4 C<default_value> when C<Default> is not active

default_value argument is still actual in the Process method even if
